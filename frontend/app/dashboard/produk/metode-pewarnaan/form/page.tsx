...
export default function FormPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const response = await PostDataApi(`/api/products/color-method`, payload);

    if (response.status === 201) {
      Notify.success("Berhasil menambahkan Metode pewarnaan");
      router.back();
    } else {
  ...
  return (
    <div className="space-y-5">
      <AppBar buttonBack title="Metode pewarnaan Baru" />
